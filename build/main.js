...
//添加自定义常量，区分大小写
var __addConstant = function __addConstant(name, value) {
    var matchs = /^[\s]*([a-zA-Z]+?[\w]*)[\s]*$/.exec(name);
    //常量只能为字母和数字组成，且首字符必须为字母！
    if (matchs === null) throw new Error("11");
    //存在同名常量？
    if (__constants.hasOwnProperty(matchs[1])) throw new Error("13");
    //值是否有效？
    var valueNum = void 0;
    try {
...
var __isTriangle = ["sin", "cos", "tan"];

var __ang2rad = function __ang2rad(a) {
    return a / 180 * Math.PI;
};
var __rad2ang = function __rad2ang(r) {
...
var _loop = function _loop(name) {
    //局部变量
    var oriFunc = __methods[name];
    var oriArgsCount = __argCount[name];
    __methods[name] = function () {
        ...
        if (args.length > oriArgsCount) {
            ...
            var ret = oriFunc.apply(null, __isTriangle.indexOf(name) >= 0 && status.isAngle ? args.map(function (a) {
                return __ang2rad(a);
            }) : args);
            if (typeof ret !== "number") throw new Error("29");else return ret;
        }
    };
};
...
var __addMethod = function __addMethod(name, oriArg, fn) {
    ...
    __methods[name] = function () {
        ...
        if (args.length > oriArgsCount) {
            ...
            var ret = fn.apply(null, __isTriangle.indexOf(name) >= 0 && status.isAngle ? args.map(function (a) {
                return __ang2rad(a);
            }) : args);
            if (typeof ret !== "number") throw new Error("29");else return ret;
        }
...
var errorMsg = __webpack_require__(6).error;

var csum = core.csum;
var addConst = core.addConst;
var addMethod = core.addMethod;
...
jQuery(document).ready(function ($) {
    ...
    $(".cal").on("click", "input[name='calButton']", function () {
        $("#csum>span").slideUp("fast");

        var str = $("input[name='calStr']").val();
        var result = csum(str);
        ...
        var result = addConst($("#constantName").val(), $("#constantValue").val());
        ...
        var arg = $("#functionArg").val();
        var body = $("#functionValue").val();
        ...
        var result = addMethod(name, arg.split(','), fn);
        if (result instanceof Error) {
            var msg = void 0,
                tmp = void 0;
            tmp = result.message.split(":");
...
var methods = __webpack_require__(1).methods;
var _addConst = __webpack_require__(0).addConstant;
var _addMethod = __webpack_require__(1).addMethod;
...
var Calculator = function Calculator() {
    ...
    //检测运算符
    var isOperation = function isOperation(value) {
        return (/[\+\-\*\/\^\%]{1}/.test(value)
        );
    };
    //检测字母，可能为常量或函数
    var isAbc = function isAbc(value) {
    ...
    //获取函数值
    var getMethod = function getMethod(func, args) {
        return methods[func].apply(methods, _toConsumableArray(args));
    };
    //删除空白字符，返回第一个不为空白字符索引（有可能返回index）
    var deleteBlanks = function deleteBlanks(str, index) {
        var pattern = /[\s]/;
        var len = str.length;
    ...
    //比较运算符的等级
    var opCompar = function opCompar(opa, opb) {
        return opLever[opa] < opLever[opb];
    };
    //合并多个连续加减符号，返回合并后的运算符。
    // 此函数会抛出错误，如“+*+”
    var mergeAdd = function mergeAdd(strtmp) {
        var ret = "+",
            tmp = void 0;
        ...
        try {
            for (var _iterator = strtmp[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                tmp = _step.value;
    ...
    function _innercsum(str) {
        ...
        while (index < len) {
            ...
            //基础运算符，后置位必须为数值、或者常量、函数。bug：“1+”
                    ...
                    //基础运算符后置位检查！
                    var _tmp = deleteBlanks(str, end);
                    if (_tmp === len || isOperation(str[_tmp]) || str[_tmp] === ")") {
                        throw new Error("33:" + beg);
                    }
                    //运算符入栈规则
                ...
                //后置位参照数字
                        ...
                        //获取变量或函数名
                        var name = str.slice(beg, end);
                        //常量或函数判断？
                        var _tmp2 = deleteBlanks(str, end);
                        //如果是函数，解析参数。并调用_innercsum计算每一项参数，将结果传入函数！
                        if (str[_tmp2] === "(") {
                            //如果没找到该函数，则抛出错误
                            if (!methods.hasOwnProperty(name)) throw new Error("24:" + beg);
                            beg = _tmp2 + 1;
                            end = beg;
                            var leftBracket = 1;
                        ...
                        //函数常量的后置位判断
                        _tmp2 = deleteBlanks(str, end);
                        if (_tmp2 !== len) {
                            if (!(isOperation(str[_tmp2]) || str[_tmp2] === ")")) {
                                throw new Error("33:" + beg);
                            }
                        }
                        //移动index至下一位待处理字符
                    ...
                    //如果为左括号，则直接入栈
                        ...
                        //如果左右圆括号数量不匹配，则抛出错误！
                                ...
                                if (tmpStack[tmpStack.length - 1] === "(") tmpStack.pop();else throw new Error("35:" + index);
                                index += 1;
                            }
        ...
        //返回计算结果，结果为number类型！
        var result = retStack.pop();
        if (typeof result !== "number")
            //表达式结果异常
            throw new Error("41:${index}");else return result;
    }
    return _innercsum;
};
...
module.exports = {
    csum: function csum(str) {
        return catchError(cal, str);
    },
    addMethod: function addMethod(name, oriArg, body) {
        //???
        return catchError(_addMethod, name, oriArg, body);
    },
    addConst: function addConst(name, value) {
        return catchError(_addConst, name, value);
    }
};
